...
export default function AdminAuthorFormPage() {
  ...
  const mutation = useMutation({
    ...
    onError: (err) => {
      ...
      setServerError(
        axiosErr.response?.data?.message ?? t('admin.authors.saveFailed'),
      );
    },
  });
  ...
  function handleSubmit(e: FormEvent) {
    ...
    setServerError('');
    if (!name.trim()) {
      setFieldError(t('admin.authors.nameRequired'));
      return;
    }
    ...
    <div style={{ minHeight: '100vh', background: theme.colors.black }}>
      ...
      <main style={{ maxWidth: 520, margin: '0 auto', padding: `${theme.spacing.xxl}px ${theme.spacing.xl}px` }}>
        ...
        <h1 style={{ color: theme.colors.white, fontSize: 28, fontWeight: 700, marginTop: theme.spacing.base, marginBottom: theme.spacing.xl }}>
          {isEdit ? t('admin.authors.editTitle') : t('admin.authors.newTitle')}
        </h1>
        ...
        <form onSubmit={handleSubmit} noValidate>
          <div style={{ marginBottom: theme.spacing.lg }}>
            <label htmlFor="author-name" style={labelStyle}>
              {t('admin.authors.nameLabel')} <span aria-hidden="true">*</span>
            </label>
            <input
              id="author-name"
              type="text"
              ...
              placeholder={t('admin.authors.namePlaceholder')}
          ...
          <div style={{ display: 'flex', gap: theme.spacing.md }}>
            ...
            >
              {mutation.isPending
                ? t('admin.authors.saving')
                : isEdit
                  ? t('admin.authors.saveButton')
                  : t('admin.authors.createButton')}
            </button>
